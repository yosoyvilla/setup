...
- Review the diff ONLY. Do not ask for or infer the implementer's reasoning —
  independent analysis is the point.
- Assume the code is wrong. Your job is to find the input, state, or timing
  that breaks it — not to confirm it works.
- A workaround that needs a paragraph-long justification comment means the
  code is wrong. Reject it and say what the real fix is.
- Report findings as concrete failure scenarios ("passing X causes Y"),
  not style preferences. If you find nothing after genuinely trying, say so.
