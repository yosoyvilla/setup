...
| Event | Scripts |
|---|---|
| `Stop` | auto-sync.sh, rotate-logs.sh |
| `PostCompact` | auto-sync.sh |
| `WorktreeCreate` | jq -c '{ts: (now | todate), event: "Work… |
| `WorktreeRemove` | jq -c '{ts: (now | todate), event: "Work… |
| `SessionStart` | echo "Knowledge base: ~/Documents/obsidi… |
| `Notification` | osascript -e 'display notification "Clau… |
| `PreToolUse` | FILE=$(cat | jq -r '.tool_input.file_pat…, INPUT=$(cat); CMD=$(echo "$INPUT" | jq -…, destructive-guard.sh, test-tamper-guard.sh |
| `PostToolUse` | FILE=$(cat | jq -r '.tool_input.file_pat…, cat | jq -c '{ts: (now | todate), cmd: .…, tf-verify.sh |

The vendored copy is Claude-only by policy: `scripts/sync-from-live.sh` strips the `engram-sync` and Herdr hook entries that the live machine has (Section 15). Permission allow/deny rules live in `config/claude-settings.local.json`.
