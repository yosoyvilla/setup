...
```markdown
## Infra Spec: <what we're changing> [project: Varsity|360latam|CedarPlanters|Kashport]
...
### Deployment Spec
```markdown
## Deployment Spec: <service> → <version> [project: Varsity|CedarPlanters|360latam]
...
- **Mixing projects in one spec**: each spec covers one project. Varsity infra and CedarPlanters app changes get separate specs even if related.
