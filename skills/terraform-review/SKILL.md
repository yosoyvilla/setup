...
- `kubernetes_manifest` (hashicorp/kubernetes) validates the CRD kind against the live cluster at PLAN time, not apply. A config that instantiates a CRD must be planned after the module that installs the CRD has been applied; `depends_on` does not defer it. (Verified live; 4/10 benchmarked models claimed the opposite.)
